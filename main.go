	...
	return registration{
		...
		Metadata: pluginapi.Metadata{
			...
			ConfigFields: []pluginapi.ConfigField{
				{Name: "proxy", Type: pluginapi.ConfigFieldTypeString, Description: "兼容配置：多个代理每行一个，也可以直接粘贴 JSON 数组。建议改用下方的 proxies 列表，避免单行输入框把多行显示成一行。"},
				{Name: "proxies", Type: pluginapi.ConfigFieldTypeArray, Description: "代理 JSON 数组，例如 [\"host:port:user:pw\", \"host2:port:user:pw\"]。每项格式为 host:port:user:password（也可省略账号密码、只写 host:port）或 http/https/socks5 URL。未命中时自动轮询下一条；单条写错只会跳过该条并记日志，不会中断整轮探测。"},
				...
				{Name: "interval_seconds", Type: pluginapi.ConfigFieldTypeInteger, Description: "自动探测间隔（秒）。默认 120，即 2 分钟：一张 292 现在只活几分钟，间隔太长就会出现大量裸发请求。"},
				{Name: "ttl_seconds", Type: pluginapi.ConfigFieldTypeInteger, Description: "已缓存 state 的有效期（秒）。默认 300，即 5 分钟——实测一张 292 只能维持约 200 秒，写成 1 小时只会让过期票被继续注入。状态页会给出实测的组合寿命，可据此调整。"},
				{Name: "inject", Type: pluginapi.ConfigFieldTypeBoolean, Description: "把缓存的 turn state 注入后续 Codex 生产请求，用于保持不降智。默认开启。"},
				{Name: "harvest", Type: pluginapi.ConfigFieldTypeBoolean, Description: "从正常 CPA Codex 流量中采集 turn state。默认开启；长度与 Fernet 块数只做展示，不再作为收不收的门槛。"},
				{Name: "probe", Type: pluginapi.ConfigFieldTypeBoolean, Description: "运行代理探测循环，主动获取 state。默认开启。"},
				{Name: "direct_probe", Type: pluginapi.ConfigFieldTypeBoolean, Description: "探测时先试不使用代理的直连；一旦拿到 state 就写入缓存并结束该轮。默认开启。配合空的代理池即为「只用直连探测」；想只用代理池则设为 false。"},
				{Name: "inject_cookies", Type: pluginapi.ConfigFieldTypeBoolean, Description: "注入账号级路由 Cookie（__cflb / __oailb）。默认开启。Cookie 与 state 相互独立：缓存里没有 state 时也会单独注入 Cookie（日志里记为来源 cookie-only）。注意 CPA 的 codex 出站白名单里没有 Cookie，需要在账号 JSON 里加 \"headers\": {\"Cookie\": \"$Cookie\"}（或给 CPA 打补丁），注入的 Cookie 才会真正到达上游。"},
				...
				{Name: "probe_schedule", Type: pluginapi.ConfigFieldTypeEnum, EnumValues: []string{"fixed", "state_aware", "on_demand"}, Description: "探测调度方式。fixed 为固定间隔；state_aware 在已持有新鲜 state 时跳过周期探测，等接近过期再续期。on_demand 无定时或启动探测，由请求触发。默认 state_aware——票只活几分钟，固定间隔很容易在到期后才发现。"},
				...
				{Name: "probe_lead_seconds", Type: pluginapi.ConfigFieldTypeInteger, Description: "state_aware / on_demand 模式下，在 state 过期前提前多少秒开始探测。默认 180；设为 0 时改用 state_refresh_seconds。"},
				...
				{Name: "attempts_per_route", Type: pluginapi.ConfigFieldTypeInteger, Description: "同一条线路（直连或某一个代理）重复尝试几次后才换下一条线路，仍受 max_probe_attempts 总次数限制。默认 1，最大 10。"},
