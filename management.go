...
func renderStatusPage(view statusView, triggered bool) []byte {
	...
	out.WriteString(".injection-row td,.probe-log-row td{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:220px}.injection-row td:hover,.probe-log-row td:hover{overflow:visible;white-space:normal;word-break:break-word}")
	...
	out.WriteString("<div class=\"section\" id=\"probe-logs-section\"><div class=\"section-title\">探测日志</div><div class=\"probe-logs-content\">")
	out.WriteString(renderProbeLogsFragment(view))
	out.WriteString("</div></div>")

	out.WriteString("<div class=\"section\" id=\"injection-section\"><div class=\"section-title\">注入记录</div><div class=\"injection-content\">")
	...
	out.WriteString("function bindSectionRefresh(btnId,sectionId,contentClass,op){var section=document.getElementById(sectionId);if(!section)return;section.addEventListener('click',function(e){var btn=e.target.closest('#'+btnId);if(!btn)return;btn.disabled=true;fetch(location.pathname+'?op='+op).then(function(r){return r.text();}).then(function(html){var content=section.querySelector('.'+contentClass);if(content){content.innerHTML=html;}if(op==='fragment_probe_logs'){initProbeLogs();}else if(op==='fragment_injections'){initInjectionTable();}}).finally(function(){var current=section.querySelector('#'+btnId);if(current)current.disabled=false;});});}bindSectionRefresh('probe-refresh','probe-logs-section','probe-logs-content','fragment_probe_logs');bindSectionRefresh('injection-refresh','injection-section','injection-content','fragment_injections');initProbeLogs();initInjectionTable();")
